...
namespace Anubis.Tests.Infrastructure.Network;
...
public class TechFingerprintServiceTests : IDisposable
{
    ...
    public async Task ScanAsync_ShouldDetectTechnologiesFromHeadersAndHtml_WhenTargetResponds()
    {
        ...
        var logger = new Mock<ILogger<Anubis.Infrastructure.Network.TechFingerprintService>>();

        // Assuming TechFingerprintService lives in Anubis.Infrastructure.Network
        // Note: For compilation to succeed in this TDD step, we use reflection or just assume the class will exist.
        // I will instantiate the interface once it's implemented.
        var service = new Anubis.Infrastructure.Network.TechFingerprintService(mockHttpClientFactory.Object, logger.Object);
