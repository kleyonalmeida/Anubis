...
using Anubis.Tests.Infrastructure.Network;
...
namespace Anubis.Tests.Application.Security;
...
public class VulnScanServiceTests : IDisposable
{
    ...
    public async Task ScanAsync_ShouldDetectVulnerabilities_WhenServerIsVulnerable()
    {
        ...
        var exploitSenderLogger = new Mock<ILogger<Anubis.Infrastructure.Network.ExploitSender>>();
        var exploitSender = new Anubis.Infrastructure.Network.ExploitSender(mockHttpClientFactory.Object, exploitSenderLogger.Object);
        var service = new VulnScanService(exploitSender, _mockLogger.Object);

        // Act
        var findings = new List<Anubis.Domain.Entities.VulnerabilityFinding>();
        ...
        gitFinding.Severity.Should().Be(Anubis.Domain.Enums.VulnerabilitySeverity.Critical);
    }
}
