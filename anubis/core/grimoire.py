...
def export_html():
    ...
    for nome in arquivos:
        try:
            ...
            pass
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    html_path = os.path.expanduser("~/anubis/reports/anubis_report_" + timestamp + ".html")
...
<title>ANUBIS REPORT """ + timestamp + """</title>
<style>
...
<body>
<div class="hd">
<h1>&#9760; ANUBIS INTELLIGENCE REPORT</h1>
...
""" + cards + """
<div class="ft"><p>ANUBIS v1.3.0 | Use only on authorized targets</p></div>
<script>
...
def export_markdown(target):
    ...
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    out = os.path.expanduser("~/anubis/reports/" + base + "_" + timestamp + ".md")
    os.makedirs(os.path.dirname(out), exist_ok=True)

    md  = "# 🔱 ANUBIS REPORT\n\n"
    md += "**Target:** `" + target + "`  \n"
    md += "**Date:** " + datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S") + "  \n"
    md += "**Tool:** Anubis v1.3.0 | github.com/lohjs-0  \n\n"
    md += "---\n\n"
